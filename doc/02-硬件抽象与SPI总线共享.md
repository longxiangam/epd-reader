...
在嵌入式开发中，"操作硬件"是最基础也是最容易出错的事情。两个模块同时操作同一个 SPI 总线？寄存器配置冲突？GPIO 被两个地方同时拉高拉低？在 C 语言中，这些问题只能靠开发者的自觉和代码审查来避免。

Rust 通过类型系统和所有权机制，将很多这类问题变成了编译期错误。本文将深入 esp-hal v1.x 的硬件抽象层设计，并以本项目中最典型的一个场景——SPI 总线共享——来展示嵌入式 Rust 如何在保证安全的前提下高效操作硬件。
...
现在到了关键问题：一个 SPI 总线上挂了两个设备（EPD 和 SD 卡），它们各有自己的片选（CS）信号。在 C 语言中，你可能会这样做：
...
但如果有两个任务同时调用呢？SPI 总线的 MOSI/SCK 线会被两个设备同时驱动，数据错乱。在 Embassy 的异步环境中，一个任务 `.await` 让出执行权时，另一个任务可能正好开始操作 SPI。
